...
//        }
        ...
        for (i = 0; i <= li.size() - 1; i++,j++) {
            ...
            Pattern pattern = Pattern.compile("\\注册资本 ：[\\u4e00-\\u9fa5\\s\\d].{1,}.");
            Pattern pattern1 = Pattern.compile("\\自 ：[\\u4e00-\\u9fa5\\s\\d]{1,}");
            Pattern pattern2 = Pattern.compile("\\经营范围 ：[\\u4e00-\\u9fa5\\s\\d].{1,}\\。");
            Pattern pattern3 = Pattern.compile("\\营业场所 ：[\\u4e00-\\u9fa5\\s\\d].{1,}|\\住所 ：[\\u4e00-\\u9fa5\\s\\d].{1,}");
            Pattern pattern4 = Pattern.compile("\\类型 ：[\\u4e00-\\u9fa5\\s\\d]{1,}");
            Pattern pattern5 = Pattern.compile("\\法定代表人 ：[\\u4e00-\\u9fa5\\s\\dA-Za-z].{1,}|\\负责人 ：[\\u4e00-\\u9fa5\\s\\dA-Za-z].{1,}");
            Pattern pattern6 = Pattern.compile("\\登记机关 ：[\\u4e00-\\u9fa5\\s\\d]{1,}");
            Pattern pattern7 = Pattern.compile("\\登记状态 ：[\\u4e00-\\u9fa5\\s\\d].{1,}");
            Pattern pattern8 = Pattern.compile("\\核准日期 ：[\\u4e00-\\u9fa5\\s\\d].{1,}");
            Pattern pattern9 = Pattern.compile("\\邮政编码：[\\u4e00-\\u9fa5\\s\\d].{1,}");
            Pattern pattern10 = Pattern.compile("\\邮箱：[\\u4e00-\\u9fa5\\s\\d].{1,}");
            Pattern pattern11= Pattern.compile("\\电话：[\\u4e00-\\u9fa5\\s\\d-]{1,}");
...
//                }
                ...
                Pattern pattern22 = Pattern.compile("\\经营范围 ：[\\u4e00-\\u9fa5\\s\\d].{1,}");
            Matcher matcher22 = pattern22.matcher(yyzz);

                if (matcher22.find()) {
                    String JYFW = matcher22.group(0);
                    FW=JYFW.split("：")[1];
                }
