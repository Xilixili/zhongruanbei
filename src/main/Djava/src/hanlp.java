...
//        }
        ...
        for (i = 0; i <= li.size() - 1; i++,j++) {
            ...
            Pattern pattern = Pattern.compile("\\注册资本：[\\u4e00-\\u9fa5\\s\\d]{1,}.[\\u4e00-\\u9fa5\\s\\d]{1,}");
...
//            sql3="insert into company1(ID,reg_no,com_name,ZB,RQ,FW,PRO)values ('"+ i+" ','"+ reg_no.trim()+" ','"+com_name.trim()+" ',' "+ZB.trim()+" ',' "+RQ.trim()+" ',' "+FW.trim()+" ',' "+PRO.trim()+" ')";
            sql3="insert into company1(ID,reg_no,com_name,ZB,RQ,FW,PRO)values ('"+j+"','"+ reg_no.trim()+"','"+com_name.trim()+"','"+ZB.trim()+"','"+RQ.trim()+"','"+FW.trim()+"','"+"上海市"+"')";
