...
function showTab(event) {
	...
	if(action != 'no'){
		...
		if(action == 'next'){
			direction = direction - (840 * (index - current));
			tab_d = tab_d - (168 * (index - current));
		}else{
			direction = direction + (840 * (current - index));
			tab_d = tab_d + (168 * (current - index));
		}
